...
`;
    ...
    expect(findings.some((f) => f.ruleId === "inject-exec" && f.severity === "critical")).toBe(
      true,
    );
  });
...
`;
    const findings = scanSource(source, "plugin.ts");
    expect(findings.some((f) => f.ruleId === "inject-exec")).toBe(false);
  });
...
`;
    const findings = scanSource(source, "plugin.ts");
    expect(
      findings.some((f) => f.ruleId === "inject-eval" && f.severity === "critical"),
    ).toBe(true);
  });
...
`;
    const findings = scanSource(source, "plugin.ts");
    expect(
      findings.some((f) => f.ruleId === "exfil-read-then-network" && f.severity === "critical"),
    ).toBe(true);
  });
...
`;
    const findings = scanSource(source, "plugin.ts");
    expect(findings.some((f) => f.ruleId === "obfusc-hex" && f.severity === "warn")).toBe(
      true,
    );
  });
...
`;
    const findings = scanSource(source, "plugin.ts");
    expect(
      findings.some((f) => f.ruleId === "obfusc-base64" && f.message.includes("base64")),
    ).toBe(true);
  });
...
`;
    const findings = scanSource(source, "plugin.ts");
    expect(findings.some((f) => f.ruleId === "exfil-websocket" && f.severity === "critical")).toBe(
      true,
    );
  });
...
`;
    const findings = scanSource(source, "plugin.ts");
    expect(findings.some((f) => f.ruleId === "cred-env-network" && f.severity === "critical")).toBe(
      true,
    );
  });
...
describe("scanDirectory", () => {
  it("scans .js files in a directory tree", async () => {
    ...
    expect(findings.some((f) => f.ruleId === "inject-eval")).toBe(true);
  });
  ...
  it("skips node_modules directories", async () => {
    ...
    const findings = await scanDirectory(root);
    expect(findings.some((f) => f.ruleId === "inject-eval")).toBe(false);
  });
...
describe("scanDirectoryWithSummary", () => {
  ...
  it("prioritizes included entry files when maxFiles is reached", async () => {
    ...
    expect(summary.scannedFiles).toBe(1);
    expect(summary.findings.some((f) => f.ruleId === "inject-eval")).toBe(true);
  });
